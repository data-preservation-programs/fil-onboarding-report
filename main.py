...
DBQS = {
    "active_or_published_daily_size": """
        SELECT DATE_TRUNC('day', sq.ts_from_epoch) AS dy, SUM((1::BIGINT << sq.claimed_log2_size) / 1024 / 1024 / 1024) AS size, COUNT(sq.claimed_log2_size) AS pieces, sq.client_id as client_id
        FROM (
            SELECT piece_id, ts_from_epoch(sector_start_rounded), claimed_log2_size, client_id
        ...
        GROUP BY DATE_TRUNC('day', sq.ts_from_epoch), client_id;
    """,
...
size_df = load_oracle(DBQS["active_or_published_daily_size"].format(fday=fday, lday=lday)).rename(
    columns={"dy": "PTime", "size": "Onchain", "pieces": "Pieces"})
size_df["Day"] = pd.to_datetime(size_df.PTime).dt.tz_localize(None)

last_week = size_df[size_df["Day"] > lday - pd.DateOffset(weeks=1)]
last_week = last_week.groupby("client_id").agg({"Onchain": "sum", "Pieces": "sum"})
last_week["Onchain"] = last_week["Onchain"] / 1024
last_week = last_week.sort_values("Onchain", ascending=False).head(10).reset_index()
last_week.set_index("client_id")

st.write("Top 10 onboarders in the last week")
st.dataframe(last_week.style.format({"Onchain": "{:,.0f} TB", "Pieces": "{:,.0f}"}), use_container_width=True)

# Set client id
client_id = extract_client_id(st.text_input("Client id"))

df = size_df[size_df['client_id'] == client_id]
st.dataframe(df, use_container_width=True)
st.metric(f"Total onboarded data by {client_id}".format(client_id=client_id), humanize(df.Onchain.sum()))
